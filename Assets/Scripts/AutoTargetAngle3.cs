...
public class AutoTargetAngle3 : MonoBehaviour
{
    ...
    private void DetectTarget()
    {
        ...
        if (!targetOn) // 락온되지 않은 경우
        {
            ...
            foreach (GameObject enemy in Enemies)
            {
                ...
                {
                    if (autoTargetAngle2.target == enemy && Enemies.Length >= 2) //가장 가까운 적이 2번의 타겟이고, 적이 2명 이상이라면
                    {
                        continue; // 그 타겟은 건너뛴다.
                    }
                    else // 잡은 타겟이 2포의 타겟이 아니거나, 적이 1명이거나 없을 경우 -> 그냥 2포와 같이 공격
                    {
