...
public class AutoTargetTest : MonoBehaviour
{
    ...
    void DetectTarget()
    {
        ...
        if ( !isLockOn ) // 락온되지 않은 경우
        {
            ...
            float shortestDistance = Mathf.Infinity; // 가장 짧은 거리 for 소형
            ...
            foreach (GameObject enemy in enemies)
            {
                if ( weaponType == 1 )
                {
                    float distanceToEnemies = Vector3.Distance(transform.position, enemy.transform.position); // 나와 적과의 거리

                    if (distanceToEnemies < shortestDistance) // 첫 원소는 쇼티스트가 무한이라 무조건 이프문 안이 돌아감
                    {
